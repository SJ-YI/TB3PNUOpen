...
local sub_idx_cmdvel,sub_idx_motorcmd
sub_idx_cmdvel=rossub.subscribeTwist('/cmd_vel')
sub_idx_motorcmd=rossub.subscribeFloat32MultiArray('/motor_cmd_vel')
...
while running do
	...
	local motor_cmd_vel=rossub.checkFloat32MultiArray(sub_idx_motorcmd)
	if motor_cmd_vel then
		print("Motor Velocity Command!",unpack(motor_cmd_vel))
		hcm.set_base_motorcmd_t(t)
		Body.set_wheel_command_velocity(motor_cmd_vel)
	end
